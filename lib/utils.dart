...
class Utils {
  static void scanText(String rawText) {
    ...
    if (text.contains(Command.email)) {
      ...
      final body = _getTextAfterCommand(text: text, command: Command.call);

      call(body: body);
    } else if (text.contains(Command.sms)) {
      final body = _getTextAfterCommand(text: text, command: Command.sms);
